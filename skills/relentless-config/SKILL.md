...
Preserve the maximum-intelligence defaults unless the user explicitly changes them: native Grok work is `grok-4.5-latest` at `max` effort; every active direct xAI fusion seat is exact Grok 4.5; and the disabled, explicitly enabled GPT option is exact GPT-5.6 Sol. Never introduce a weaker fallback or silent model degradation.
